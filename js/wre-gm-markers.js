				...
				content: '<div class="map-marker-wrapper">' +
							...
							'<div class="map-marker-container">' +
								...
								'<div class="arrow-down"></div>' + 
									...
									'<p class="price">' + markerData[i].price + '</p>' +
									'<p class="content">' + markerData[i].content + '</p>' + 
							'</div>' +
							'</a>' + 
						'</div>',
				...
				return function () {
					if (newMarkers[i].infobox.getVisible()) {
						...
						for (j = 0; j < newMarkers.length; j++) {
						  newMarkers[j].infobox.hide();
						}
					} else {  
						var j;
						for (j = 0; j < newMarkers.length; j++) {
						  	if (newMarkers[j].infobox.getVisible()) {
						  		newMarkers[j].infobox.hide();
							} 
							setTimeout(function(){ newMarkers[i].infobox.show(); }, 1000);
						}
